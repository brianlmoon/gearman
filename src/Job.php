...
use Moonspot\Gearman\Job\Exception;
...
if (!defined('NET_GEARMAN_JOB_PATH')) {
    define('NET_GEARMAN_JOB_PATH', 'Moonspot/Gearman/Job');
}

// Define this if you want your Jobs to have a prefix requirement
if (!defined('NET_GEARMAN_JOB_CLASS_PREFIX')) {
    define('NET_GEARMAN_JOB_CLASS_PREFIX', 'Moonspot\\Gearman\\Job\\');
}
...
abstract class Job
{
     ...
     *
     * The Moonspot\Gearman\Worker class creates connections to multiple job servers
     ...
     *
     * @return Common Instance of Job\Common child
     * @throws Exception
     * @see Job\Common
     */
    static public function factory(string $job, Connection $conn, string $handle, array $initParams = array()): Job\Common
    {
        ...
        if ( ! file_exists($file) ) {
            throw new Job\Exception('Invalid Job class file: ' . (empty($file) ? '<empty>' : $file));
        }
        ...
        if (!class_exists($class)) {
            throw new Job\Exception('Invalid Job class: ' . (empty($class) ? '<empty>' : $class) . ' in ' . (empty($file) ? '<empty>' : $file) );
        }

        $instance = new $class($conn, $handle, $initParams);
        if (!$instance instanceof Job\Common) {
            throw new Job\Exception('Job is of invalid type: ' . get_class($instance));
        }
